function solution(span, h, events) {
    var stories = [],
        return_stories = [],
        i, j, k,
        event, sets,
        time, score, height,tmp,
        Old, New,
        max, list
    ...
    for(i in events) {
        if(events[i].length == 3) {
                ...
                for(j in sets) {
                    ...
                    if(k <= h) {
                        ...
                        if(!Old) {
                            ...
                            tmp[k] = New
                        } else if(Old.score === New.score && Old.list.length === New.list.length && Old.list > New.list) {
                            tmp[k] = New
                        }
                    }
                }
            ...
            return_stories.push([max, ...list.split(",").map(a => a - 99).slice(1)])
        }
    }
